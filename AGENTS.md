...
- 所有特性和改动必须以**高性能、稳定性、不卡顿**为前提，这是第一性原则。
- 任何新功能都不应以牺牲交互流畅性、响应速度、内存占用稳定性或长时间运行可靠性为代价。
- 涉及终端渲染、文件传输、目录浏览、会话管理、列表加载、搜索过滤等高频路径时，优先选择更低开销、更少阻塞、更易预测的实现。
- 默认避免在 UI 主线程执行重计算、阻塞 I/O、同步网络/磁盘操作或大规模状态刷新。
- 如功能方案与性能/稳定性冲突，优先保留性能与稳定性，并采用更保守、更渐进的实现。
- 对于功能修改尽量做最小修改，避免引入额外复杂度、隐藏性能回退或新的卡顿风险。
